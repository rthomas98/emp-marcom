...
export function CompanyHeader({
    ...
    description = "Since 2009, Empuls3 has helped organizations improve websites, applications, and business systems. Our remote, senior-led team works directly with clients to understand the operating problem and deliver a practical path forward.",
    imagesPartOne = defaultImagesPartOne,
    imagesPartTwo = defaultImagesPartTwo,
}: Partial<Props>) {
    return (
        <section id="company-header" className="relative px-[5%]" aria-labelledby="company-header-heading">
            <div className="flex max-h-[60rem] min-h-svh items-center">
                <div className="container mx-auto py-16 md:py-24 lg:py-28">
                    <header className="relative z-10 mx-auto max-w-3xl text-center">
                        <h1 id="company-header-heading" className="mb-5 text-5xl font-bold text-white md:mb-6 md:text-6xl lg:text-7xl">
                            {heading}
                        </h1>
                        <p className="md:text-md text-white">{description}</p>
                        <nav className="mt-6 flex items-center justify-center gap-x-4 md:mt-8" aria-label="Company navigation">
                            <Link
                                href="/company/about"
                ...
                <div className="absolute inset-0 z-0 overflow-hidden" aria-hidden="true" role="presentation">
...
const ImageGrid = ({ images, animation }: { images: ImageProps[]; animation: string }) => (
    <div className={`flex flex-col gap-4 ${animation}`}>
        {images.map((image, index) => (
            <div key={index} className="relative mb-4 w-full pt-[120%]">
                <img className="absolute inset-0 h-full w-full rounded-md object-cover" src={image.src} alt={image.alt} />
            </div>
        ))}
        {/* Duplicate images for seamless loop */}
        {images.map((image, index) => (
            <div key={`duplicate-${index}`} className="relative mb-4 w-full pt-[120%]">
                <img className="absolute inset-0 h-full w-full rounded-md object-cover" src={image.src} alt={image.alt} />
            </div>
        ))}
    </div>
