...
export default function SiteLayout({ children, title = 'Empuls3' }: SiteLayoutProps) {
    ...
    const canonicalUrl = `https://www.empuls3.com${canonicalPath === '/' ? '/' : canonicalPath.replace(/\/$/, '')}`;
    ...
    return (
        <div className="flex min-h-screen w-full flex-col bg-white bg-[linear-gradient(to_right,#80808012_1px,transparent_1px),linear-gradient(to_bottom,#80808012_1px,transparent_1px)] bg-[size:24px_24px]">
            <Head title={title}>
                <link rel="canonical" href={canonicalUrl} />
            </Head>

            {/* Particles Background */}
            <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none', zIndex: 0 }}>{particles}</div>
